...
`<script src>` 載入。POI（景點 / 神社 / 寺廟）目前**仍** runtime fetch，雙擊 file:// 會失敗
而不顯示 — 沒抱怨先放著。要全 self-contained 也可仿 prefectures 預生成。
...
- **POI overlay**（⛩ / 卍 / ★）— 三個獨立 layerGroup（`shrineLayer / templeLayer / attrLayer`），
  從 Overpass runtime fetch
